...
let taco = inventors.filter(fun => (fun.year >= 1500 && fun.year <= 1599))
//console.table(taco);

  // Array.prototype.map()
  // 2. Give us an array of the inventors first and last names
let firstAndLast = inventors.map(fullName => (fullName.first + fullName.last))
...
let oldToYoung = inventors.sort((a, b) => a.year > b.year ? 1 : -1)
  ...
  let totalInventorYears = inventors.reduce((a, b) => {
    return a + (b.passed - b.year);
  }, 0);

 // console.log(totalInventorYears);
    
  // 5. Sort the inventors by most years lived
...
let byLastName = people.sort((y, z) => {
    const [aLast, aFirst] = y.split(', ');
    const [bLast, bFirst] = z.split(', ');
